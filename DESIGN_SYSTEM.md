...
>
> **Source of truth:** [`app/src/theme.js`](app/src/theme.js) for tokens,
> [`app/src/components/Primitives.js`](app/src/components/Primitives.js) for
> components. This doc mirrors those files — if you change a token, change it
> there and here.
...
All values from [`app/src/theme.js`](app/src/theme.js).
...
> currently applied only in [`EarningsChart.js`](app/src/components/EarningsChart.js).
...
Scale (`typography` in [`app/src/theme.js`](app/src/theme.js), all `fontFamily: 'System'`):
...
[`app/src/theme.js`](app/src/theme.js):
...
From [`app/src/components/Primitives.js`](app/src/components/Primitives.js).
...
[`app/src/components/BottomNav.js`](app/src/components/BottomNav.js).
...
[`app/src/screens/KartScreen.js`](app/src/screens/KartScreen.js).
...
[`AktivitetScreen.js`](app/src/screens/AktivitetScreen.js).
...
[`HostSpotCard.js`](app/src/components/HostSpotCard.js).
...
- **Currency:** Norwegian kroner. Hourly price unit is `kr/t` (kroner per time).
  Format amounts with `formatKr()` from [`lib/format.js`](app/src/lib/format.js).
...
> rather than importing from [`theme.js`](app/src/theme.js). That's the existing
...
When starting a design task in a new session, point Claude at this file:
*"Follow `DESIGN_SYSTEM.md` — match the principles, signature patterns, and
copy conventions, not just the tokens."* Sections 9–12 are what make output look
like *this* app rather than generic dark mode.
